package dev.december.jeterbackend.client.features.files.data.services

import dev.december.jeterbackend.client.features.authorization.domain.errors.RefreshTokenFailure
import dev.december.jeterbackend.client.features.files.domain.services.FileService
...
internal class FileServiceImpl(
    ...
    override suspend fun storeFile(directory: FileDirectory, part: FilePart, priority: Int, contentLength: Long): Data<File> {
        ...
        return try {
            ...
            ex.printStackTrace()
            Data.Error(RefreshTokenFailure())//FileStoreFailure()
        }
    }
    ...
    override suspend fun getFile(directory: FileDirectory, id: String, format: String): Data<Resource> {
        ...
        return if (targetLocation.exists() && !targetLocation.isDirectory()) {
            ...
            Data.Error(RefreshTokenFailure())//FileNotFoundFailure()
        }
    }
    ...
    override suspend fun updateFile(id: String, directory: FileDirectory?, part: FilePart?, priority: Int?): Data<File>{
        return try {
            if (directory == null && part == null){
                return Data.Error(RefreshTokenFailure())//FileNotFoundFailure()
            }
            withContext(dispatcher) {
                val oldEntity = fileRepository.findByIdOrNull(id)
                    ?: return@withContext Data.Error(RefreshTokenFailure())//FileNotFoundFailure()
